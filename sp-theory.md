...
- EAGAIN or EWOULDBLOCK - файловия дискриптор се отнася до сокет и е маркиран като неблокиращ и четенето ще блокира.
- EBADF - файловия дескриптор не е валиден или не е отворен за четене
- EFAULT - buf е извън достъпното ядресно пространство
- EINTR - повикването е било прекъснато от сигнал преди да има прочетени данни
- EINVAL - fd е свързан с обект, който не е подходящ за четене
- EINVAL - fd е бил създаен чрез повикване на timerfd_create(2) и е даден грешен размер на буфера
- EIO - ако е пуснат на фонов режим
- EISDIR - файловия дескриптор fd сочи към директория

# write
ssize_t write(int fd, const void buf[.count], size_t count);

пише във файлов дескриптор

## описание

write() записва до броене на байтове от буфера, започвайки от buf към файла, посочен от файловия дескриптор fd.

## връща

При успех се връща броят на записаните байтове. При грешка, връща се -1 и errno е настроено да указва грешката.
